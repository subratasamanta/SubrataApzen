...
function bindAnyScroll(handler) {
	if (this.addEventListener) {
		this.addEventListener('scroll', handler, true); // useCapture=true
		return true;
	}
	return false;
}
...
function unbindAnyScroll(handler) {
	if (this.removeEventListener) {
		this.removeEventListener('scroll', handler, true); // useCapture=true
		return true;
	}
	return false;
}
...
var Popover = Class.extend(ListenerMixin, {
	...
	position: function() {
		...
		if (viewportEl.is(this) || viewportEl.is(document)) { // normalize getScrollParent's result
			viewportEl = thisEl;
			viewportTop = 0; // the this is always at the top left
		...
		// if the this is scrolled, it causes the visible area to be further down
		viewportTop += thisEl.scrollTop();
		viewportLeft += thisEl.scrollLeft();
...
TimeGrid.mixin({
	...
	// Works when called after initial render, after a this resize/zoom for example.
...
function Calendar_constructor(element, overrides) {
	...
	var thisResizeProxy; // wraps the thisResize function
	var ignorethisResize = 0;
	var date; // unzoned
	...
	function initialRender() {
		...
		if (t.options.handlethisResize) {
			thisResizeProxy = debounce(thisResize, t.options.thisResizeDelay); // prevents rapid calls
			$(this).resize(thisResizeProxy);
		}
	}
	...
	function destroy() {
		...
		if (thisResizeProxy) {
			$(this).unbind('resize', thisResizeProxy);
		}
	}
	...
	function renderView(viewType, forcedScroll) {
		...
		if (currentView) {
			...
			if (
				...
				!( // NOT within interval range signals an implicit date this change
	...
	function thisResize(ev) {
		if (
			!ignorethisResize &&
			ev.target === this && // so we don't process jqui "resize" events that have bubbled up
			currentView.start // view has already been rendered
		) {
			if (updateSize(true)) {
				currentView.publiclyTrigger('thisResize', _element);
			}
		}
	}
...
Calendar.defaults = {
	...
	handlethisResize: true,
	thisResizeDelay: 100, // milliseconds before an updateSize happens
...
function EventManager() { // assumed to be a calendar
	...
	// HACK: can override the recurring this by providing custom rangeStart/rangeEnd (for businessHours).
	...
	handleSegClick: function(seg, ev) {
		...
		if (!$(ev.target).closest('a[href]').length) {
			...
			if (url && !ev.isDefaultPrevented()) { // jsEvent not cancelled in handler
				this.location.href = url; // simulate link click
			}
		}
	},
